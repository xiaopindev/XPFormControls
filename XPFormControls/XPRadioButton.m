...
- (void)buttonClicked:(UIButton*)button{
    ...
    //改变按钮的状态
    if(self.radioStyle == XPRadioStyleNoText){
        UIImage *image = self.checked ? [UIImage imageNamed:@"xpf_rb_2_1"]:[UIImage imageNamed:@"xpf_rb_2_0"];
        [self.button setBackgroundImage:image forState:UIControlStateNormal];
    } else if(self.radioStyle == XPRadioStyleImageText){
        self.imgView.image = self.checked ? [UIImage imageNamed:@"xpf_rb_2_1"]:[UIImage imageNamed:@"xpf_rb_2_0"];
    } else if(self.radioStyle == XPRadioStyleCustom){
